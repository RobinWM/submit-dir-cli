...
`ship` 封装 aidirs.org / backlinkdirs.com 的提交 API，提供这些命令：

- `login` — 浏览器授权，自动获取并存储 Token
- `submit <url>` — 提交 URL
- `fetch <url>` — 仅预览网站元数据，不提交
- `version` — 查看当前版本，可选检查最新 release
- `self-update` — 自更新到最新 release（Windows 目前提示手动下载）

凭证存储在 `~/.config/ship/config.json`。
