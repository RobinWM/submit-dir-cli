...
export async function submit(cliVersion: string, targetUrl: string, options: { site?: string; json?: boolean; quiet?: boolean }) {
  try {
...
export async function fetchPreview(cliVersion: string, targetUrl: string, options: { site?: string; json?: boolean; quiet?: boolean }) {
  try {
    await maybeNotifyUpdate(cliVersion, { json: options.json, quiet: options.quiet });
    const validUrl = validateUrl(targetUrl);
    const config = await loadConfig({ site: options.site });
